    ...
    if (
        ...
        switch (reaction.emoji.name) {
            case "🎟️":
                ...
                if (reaction.message.channel.id !== "861310466503868456") return;
                reaction.users.remove(user);
                let username = user.username;
                if (member.roles.cache.some(role => ["VIP"].includes(role.name))) {
                    // VIP
                    var catergoryID = "861310096477913098"; // 770765272683905076
                } else {
                    // NON VIP
                    var catergoryID = "861310096477913098"; // 715703317996765204
                }
            ...
            case "✅":
                if (message.channel.name.startsWith("ticket")) {
                    ...
                    // let username = message.channel.name.split("-")[1]
                    // message.channel.updateOverwrite(username, {"VIEW_CHANNEL": false})
                }
